...
def solve_merton_system(E, sigma_E, D, r, T, method='hybrid'):
    ...
    # ========== STEP 1: Scaling to Prevent Solver Asphyxiation ==========
    # SciPy solvers fail when calculating residuals on trillions of dollars.
    # We scale the inputs down to billions to keep the math stable.
    scale = 1e9
    E_s = E / scale
    D_s = D / scale

    # ========== STEP 2: Defining the System of Equations ==========
    def equations(vars):
        V_s, sigma_V = vars
        if V_s <= 0 or sigma_V <= 0:
            return [1e10, 1e10]
        
        try:
            d1 = (np.log(V_s / D_s) + (r + 0.5 * sigma_V**2) * T) / (sigma_V * np.sqrt(T))
            d2 = d1 - sigma_V * np.sqrt(T)
            
            theoretical_E_s = V_s * norm.cdf(d1) - D_s * np.exp(-r * T) * norm.cdf(d2)
            
            if E_s <= 0:
                return [1e10, 1e10]
            
            theoretical_sigma_E = (V_s / E_s) * norm.cdf(d1) * sigma_V
            return [theoretical_E_s - E_s, theoretical_sigma_E - sigma_E]
    ...
    # ========== STEP 3: Multi-Guess Strategy ==========
    # We try multiple starting points to handle both safe and distressed firms
    guesses = [
        [E_s + D_s, sigma_E * (E_s / (E_s + D_s))],             # Standard Guess
        [E_s + D_s * 0.8, 0.25],                                # Distressed Guess
        [E_s + D_s * 0.5, sigma_E * 0.5],                       # High Leverage Guess
        [E_s + D_s, 0.40]                                       # High Volatility Guess
    ]
    
    # ========== STEP 4 & 5: Iterative Solver ==========
    for initial_guess in guesses:
        ...
        if method in ['fsolve', 'hybrid']:
            try:
                with warnings.catch_warnings():
                    ...
                    V_s, sigma_V = solution[0]
                    residuals = np.abs(solution[1]['fvec'])
                    
                    # Residuals are in billions. 1e-4 allows for acceptable numeric drift.
                    if np.all(residuals < 1e-4) and V_s > 0 and sigma_V > 0:
                        return V_s * scale, sigma_V, 'fsolve'
            except:
                continue
        ...
        if method in ['optimize', 'hybrid']:
            try:
                bounds = [(E_s * 0.1, (E_s + D_s) * 5), (0.01, 3.0)]
                ...
                if result.success and result.fun < 1e-4:
                    V_s, sigma_V = result.x
                    if V_s > 0 and sigma_V > 0:
                        return V_s * scale, sigma_V, 'optimize'
            except:
                continue

    # ========== STEP 6: Institutional Fallback ==========
    # If all solvers fail, we trust the naive asset volatility.
    # We explicitly removed the hardcoded 15% floor to prevent destroying 
    # the math for highly-levered, safe entities like mega-banks.
    V_fallback_s = E_s + D_s
    naive_sigma_V = sigma_E * (E_s / (E_s + D_s)) if (E_s + D_s) > 0 else 0.2
    
    return V_fallback_s * scale, max(0.01, naive_sigma_V), 'fallback'
